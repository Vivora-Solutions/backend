...
export const getSalonsByServiceName = async (req, res) => {
  try {
    ...
    if (!name || typeof name !== "string") {
      return res
        .status(400)
        .json({ error: "Service name is required in query" });
    }
...
export const fetchSalonServices = async (req, res) => {
  ...
  if (!salonId) {
    return res
      .status(400)
      .json({ error: "Salon ID is required in path params." });
  }
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: "Failed to fetch services for this salon." });
  }
};
...
export const fetchAvailableTimeSlotsSithum = async (req, res) => {
  try {
    const { service_ids, stylist_id, salon_id, date } = req.body;
    ...
    if (
      ...
      return res.status(400).json({
        error:
          "Missing or invalid input: service_ids (array), stylist_id, salon_id, or date are required.",
      });
    }

    const slots = await getAvailableTimeSlotsSithum({
      service_ids,
      stylist_id,
      salon_id,
      date,
    ...
    return res.status(200).json({
      ...
      workstation_info:
        slots.length > 0
          ? {
              single_workstation_slots: slots.filter(
                (s) => s.workstation_strategy === "single_workstation"
              ).length,
              multiple_workstation_slots: slots.filter(
                (s) => s.workstation_strategy === "multiple_workstations"
              ).length,
            }
          : null,
    });
    ...
    if (error.message.includes("No workstations")) {
      errorMessage = error.message;
      statusCode = 400;
    } else if (
      error.message.includes("not scheduled") ||
      error.message.includes("on leave")
    ) {
      errorMessage = error.message;
      statusCode = 400;
    }
