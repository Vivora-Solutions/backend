import {
    ...
    handleCreateWorkingStation, handleEditWorkingStation,
    handleGetAllWorkingStations, handleGetServicesOfWorkingStation
...
export const addOrEditWorkingStationServices = async (req, res) => {
    try {
        ...
        const result = await handleAddOrEditServicesToWorkingStation(user_id, station_id, service_ids);
        res.status(200).json(result);
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
