...
export const handleDeleteBooking = async (user_id, booking_id) => {
  ...
  try {
    ...
    if (booking.user_id !== user_id) {
      throw new Error("Unauthorized: You cannot delete someone else's booking.");
    }

    // 3. Start deletion inside a transaction using Supabase `rpc` (Postgres function)
    const { error: transactionError } = await client.rpc("delete_booking_transaction", {
    ...
    throw new Error(err.message || "Something went wrong while deleting the booking.");
  }
};
