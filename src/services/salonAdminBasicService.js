...
export const handleUpdateSalonDetails = async (user_id, fieldsToUpdate) => {
  ...
  const { is_approved, location, ...sanitizedFields } = fieldsToUpdate;
  ...
  const updateFields = { ...sanitizedFields, updated_at: new Date() };
  if (locationWKT) {
    updateFields.location = locationWKT;
  }
...
export const handleUpdateBannerImage = async (
  user_id,
  imageId,
  newImageLink
) => {
