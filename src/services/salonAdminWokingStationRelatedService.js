...
export const handleGetServicesOfWorkingStation = async (
  ...
  // Ensure the station is in this salon - use workstation_id to match database
  const { data: station, error: stationErr } = await supabase
    .from("workstation")
    .select("workstation_id")
    .eq("workstation_id", station_id) // Changed from station_id to workstation_id
    .eq("salon_id", salon_id)
    .single();
...
export const handleAddOrEditServicesToWorkingStation = async (
  ...
  // Ensure the station belongs to the salon - use workstation_id to match database
  const { data: station, error: stationErr } = await supabase
    .from("workstation")
    .select("workstation_id")
    .eq("workstation_id", station_id) // Changed from station_id to workstation_id
  ...
  const inserts = filteredServiceIds.map((service_id) => ({
    workstation_id: station_id, // Changed from station_id to workstation_id
    service_id,
    salon_id,
    created_at: new Date(),
  }));
