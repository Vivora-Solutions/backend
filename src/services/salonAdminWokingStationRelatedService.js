...
export const handleEditWorkingStation = async (user_id, station_id, station_name) => {
    const salon_id = await getSalonIdByAdmin(user_id);
...
export const handleGetServicesOfWorkingStation = async (user_id, station_id) => {
    const salon_id = await getSalonIdByAdmin(user_id);
...
export const handleAddOrEditServicesToWorkingStation = async (user_id, station_id, service_ids) => {
    const salon_id = await getSalonIdByAdmin(user_id);
    ...
    const filteredServiceIds = service_ids.filter(id => validServiceIds.includes(id));
